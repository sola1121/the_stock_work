...
def run_window_main(trans):
        ...
        def check_id():
            try:
                ...
                if result and ('0' in str(status)):
                    ...
                    cost = link.select_para(sql_pur3, (purchase_id,))
                    quan = link.select_para(sql_pur7, (purchase_id,))
                    var_costs.set(cost)
                    var_quan.set(quan)
                else:
    ...
    def update_ware():
        ...
        def change_info():
            try:
                para = {
                    'stock_id': var_id.get(),
                    'costs_': var_costs.get(),
                    'ware_clerk': var_clerk.get(),
                }
                link.conn()
    ...
    def search_order():
            ...
            try:
                ...
                if ck_num2==1 and ck_num1==1:
                    ...
                    para = {
                        ...
                        'rent_date': '%'+str(var_date.get())+'%',
                        'rent_price': '%'+str(var_price.get())+'%',
        ...
        btn_confirm = tkinter.Button(search_top, text="Confirm", command=search_ord).place(x=50, y=240)
    ...
    def return_stock():
        ...
        var_bdate = tkinter.StringVar()
        var_bdate.set('当前时间')
        var_price = tkinter.StringVar()
        var_de = tkinter.StringVar()

        entry1 = tkinter.Entry(return_top, textvariable=var_bdate).place(x=100, y=60)
        ...
        def check_id():
            try:
                link.conn()
                re_reprice = link.select_para(sql_ord3, (var_id.get(),))
                re_reuid = link.select_para(sql_ord4, (var_id.get(),))
                re_stid = link.select_para(sql_ord5, (var_id.get(),))
                re_delman = link.select_para(sql_ord6, (var_id.get(),))
                re_status = link.select_para(sql_ord7, (var_id.get(),))

                var_price.set(re_reprice)
                var_uid.set(re_reuid)
                var_stid.set(re_stid)
                var_de.set(re_delman)
                var_status.set(re_status)
        ...
        def return_st():
            try:
                rent_id = var_id.get().strip()
                link.conn()
                if var_bdate.get() == '当前时间':
                    para = {
                        'rent_id': rent_id,
                        'rent_price': var_price.get(),
                        'back_date': var_bdate.get(),
                        'delman': var_de.get(),
                    }
                    if link.insert_update_para(sql_ord_update2, para):
                        if link.insert_update_para(sql_ord_stock, (var_stid.get(),)):
                            pass
                        else:
                            messagebox.showerror(title='错误', message='向仓库发出请求出错，但对租赁表更改完成，\n请使用终端手动更改。')
                    else:
                        messagebox.showwarning(title='警告', message='向表单发出请求出错，请检查。')
                else:
                    para = {
                        'rent_id': rent_id,
                        'rent_price': var_price.get(),
                    ...
                    if link.insert_update_para(sql_ord_update1, para):
                        if link.insert_update_para(sql_ord_stock, (var_stid.get(),)):
                            pass
                        else:
                            messagebox.showerror(title='错误', message='向仓库发出请求出错，但对租赁表更改完成，\n请使用终端手动更改。')
    ...
    def search_cus():
        ...
        label4 = tkinter.Label(search_top, text='余额：').place(x=20, y=100)
        ...
        entry4 = tkinter.Entry(search_top, textvariable=var_balance).place(x=110, y=100)
    ...
    def update_cus():
        ...
        label3 = tkinter.Label(change_top, text='余额：').place(x=30, y=120)
        ...
        entry3 = tkinter.Entry(change_top, textvariable=var_balance).place(x=100, y=120)
        ...
        def check_uid():
            try:
                ...
                re_balance = link.select_para(sql_cus4, (var_uid.get(),))
                ...
                var_balance.set(str(re_balance)[3:-4])
        ...
        def change_info():
            try:
                para = {
                    'user_id': var_uid.get().strip(),
                    'user_name': var_name.get().strip(),
                    'sex_': var_gender.get().strip(),
                    'balance': float(var_balance.get().strip()),
                    'user_pwd': var_passwd.get().strip(),
                    'user_address': var_address.get().strip()
                }
                link.conn()
                ...
                if result:
                    ...
                    messagebox.showwarning(title='警告', message='出错。')
                link.close_conn()
            except:
                info = sys.exc_info()
                info1 = list(info)
